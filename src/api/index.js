...
// 对设备管理器初始化-添加外部监听
const connectSocket = async (ip, port, id, targetObj, propName) => {
	const socket = window.useModbusAPI.connect(ip, port, id);
	const device = await socket.connect();
	
	device.on('connect', () => {
		targetObj[propName] = device;
		console.log(`${ip}:${port} ${propName} id:${id} 连接成功`);
	});
	
	device.on('error', (err) => {
		targetObj[propName] = null;
		console.log(`${ip}:${port} ${propName} id:${id} 连接失败，错误信息:`, err);
	});
	
	device.on('disconnect', () => {
		targetObj[propName] = null;
		console.log(`${ip}:${port} ${propName} id:${id} 连接断开`);
	});
	
	return device;
};

export const initDeviceManage = async () => {
	...
	try {
		// 使用Promise.all进行并行初始化
		const initPromises = DeviceManage.deviceList.map(async item => {
			const editSocketPromise = connectSocket(item.ip, item.port, 9, item, 'editSocket');
			
			const sensorsPromises = Array.isArray(item.sensorsData) ? Array.from({length: 6}, (_, idx) => {
				const id = idx + 1;
				return connectSocket(item.ip, item.port, id, (item.sensorsData[id - 1] || {}), 'deviceSocket');
			}) : [];
			
			await Promise.all([editSocketPromise, ...sensorsPromises]);
		});
...
export const addDevice = async (ip, port, name) => {
	...
	try {
		...
		const promises = Array.from({length: 6}, async (_, idx) => {
			const id = idx + 1;
			const deviceSocket = window.useModbusAPI.connect(ip, port, id);
		...
		// 创建编辑socket
		const editDeviceSocket = window.useModbusAPI.connect(ip, port, 9);
...
export const openDevice = async (index) => {
	...
	const handleDeviceConnection = async (id, isEditSocket = false) => {
		let targetSocket, targetDevice;
		
		try {
			targetSocket = window.useModbusAPI.connect(deviceInfo.ip, deviceInfo.port, id);
			targetDevice = await targetSocket.connect();
			
			targetDevice.on('connect', () => {
				if (isEditSocket) {
					deviceInfo.editSocket = targetDevice;
				} else {
					deviceInfo.sensorsData[id - 1] = deviceInfo.sensorsData[id - 1] || {};
					deviceInfo.sensorsData[id - 1].deviceSocket = targetDevice;
				}
 ...
 */
export const updateSubstationData = async (substation) => {
	// 遍历所有的控制板
	for (let boardIndex = 0; boardIndex < substation.sockets.length; boardIndex++) {
		const socket = substation.sockets[boardIndex];
		...
		try {
			...
			for (let sensorIndex = 0; sensorIndex < 5; sensorIndex++) {
				substation.sensorsData[boardIndex][sensorIndex].current_data.temperature_data = floatData[sensorIndex * 2];
				substation.sensorsData[boardIndex][sensorIndex].current_data.vibration_data = floatData[sensorIndex * 2 + 1];
			}
		} catch (error) {
